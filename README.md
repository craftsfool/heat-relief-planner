...
1. Standard Queenstown scenario/time combinations load versioned JSON solutions
   generated by GitHub Actions.
2. Subzone and other custom configurations are submitted to Vercel Queues and
   stored in Vercel Blob when complete.
...
Precompute one standard solution locally with:

```bash
npm run precompute:solution -- --scenario=baseline --time=afternoon
```

The GitHub workflow at `.github/workflows/precompute-solutions.yml` generates
all nine standard scenario/time combinations and commits the refreshed cache
under `public/solutions`.
