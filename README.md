...
Global plans use three execution tiers:

1. The weighted Queenstown model and all 15 subzones load versioned JSON
   solutions generated by GitHub Actions.
2. Nonstandard budgets or missing cache entries are submitted to Vercel Queues
   and stored in Vercel Blob when complete.
3. If neither source is available, the browser partitions the full traversal
   across up to four Web Workers and performs local exact refinement.
...
Precompute the weighted Queenstown solution locally with:

```bash
npm run precompute:solution
```

The GitHub workflow at `.github/workflows/precompute-solutions.yml` generates
one weighted solution for Queenstown and each of its 15 subzones, then commits
the 16 refreshed cache files under `public/solutions`.
